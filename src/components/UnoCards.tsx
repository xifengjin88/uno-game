...
export interface UnoCardProps extends CardProps {
  size?: CardSize;
  style?: CSSProperties;
  onClick?: () => void;
}
...
const PALETTE: Record<CardColor, { bg: string; text: string }> = {
...
const CORNER_LABEL: Partial<Record<CardType, string>> = {
  skip:    "⊘",
  reverse: "⇄",
  draw2:   "+2",
  wild4:   "+4",
...
function getCornerLabel(type: CardType, value?: number): string {
...
function WildOval({ size, label }: { size: CardSize; label?: string }) {
  const s = SIZES[size];
  return (
    <div style={{
      width: s.ovalW,
...
export function UnoCard({
  ...
  return (
    ...
    >
      ...
      {/* Center oval */}
      {isWild
        ? <WildOval size={size} label={type === "wild4" ? "+4" : undefined} />
...
export function buildDeck(): CardProps[] {
